...
public class EclipticonViewer extends Viewer implements SelectionListener {
	...
	private boolean	_testMode			= true;
	private Table	_tablePoints		= null;
	...
	private void createManualTab( CTabFolder tabFolder ) {
		...
		percLbl.setText( "Probability:" );

		Text txtPerc = new Text( groupProperties, SWT.BORDER );
		gridData = new GridData( GridData.FILL_HORIZONTAL | GridData.VERTICAL_ALIGN_BEGINNING );
		txtPerc.setLayoutData( gridData );
	...
	public void fillTable( ArrayList<InstrumentationPoint> instrPoints ) {
		...
		// Need to fix bundle importing before connecting the two componenets.
		...
		// item.setText( 1, currentPoint.getSource() );
